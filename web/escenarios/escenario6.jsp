...
<head>
    ...
    <style>
        ...
        * {
        ...
        .est.on  { background: #facc15; }
        .est.off { background: rgba(250,204,21,0.3); }
        .nav-temp {
            font-family: 'Playfair Display', serif;
            font-size: 0.85rem;
            font-style: italic;
        ...
        .led-light.verde { background: #22c55e; box-shadow: 0 0 6px #22c55e; }
        .led-light.naranja { background: #f97316; box-shadow: 0 0 8px #f97316; animation: pulse 0.8s infinite; }
        .led-light.rojo { background: #ef4444; box-shadow: 0 0 10px #ef4444; animation: pulse 0.4s infinite; }
        @keyframes pulse {
    ...
    <div class="contenido">
        ...
        <div class="sec-bloque">
            <div style="padding: 20px;">
                <p style="display:flex; gap:12px; align-items:center; margin-bottom: 16px;">📚 <strong>Quiz Integrador</strong> — 10 preguntas sobre fuerzas intermoleculares, presión de vapor y tecnologías de cocina.</p>
                <a href="${pageContext.request.contextPath}/quiz?escenario=<%= idEscenario %>" class="btn-quiz">🏁 Iniciar Quiz Integrador →</a>
            </div>
            <footer>⚡ La olla a presión aumenta el punto de ebullición. Ley de Clausius‑Clapeyron: ln(P2/P1) = (ΔHvap/R)(1/T1 - 1/T2)</footer>
        </div>
    </div>
        ...
        function dibujarOlla() {
            ...
            particulas.forEach(p => {
                ...
                if (p.esVapor) {
                    ...
                    p.x += p.vx;
                    p.y += p.vy;
                    if (p.x < 125 || p.x > canvas.width-125) p.vx *= -1;
                    if (p.y < nivelLiquido) p.vy = Math.abs(p.vy);
                    if (p.y > 310) p.vy = -Math.abs(p.vy);
                }
            });
            if (pres > 1.5) {
                for(let i=0;i<18;i++) {
