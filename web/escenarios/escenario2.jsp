...
<body>
    ...
    <div class="contenido">
        ...
        <div class="sec-head">
            ...
            <span class="sec-titulo">📝 Evaluación — ¿Dominas la Sala de Salsas?</span>
            <div class="sec-linea"></div>
        </div>
        <div class="sec-bloque">
            <div class="quiz-wrap">
                <div>
                    <div class="quiz-titulo" style="font-family:'Playfair Display',serif; font-size:1.1rem;">Quiz — Dipolos y emulsiones</div>
                    <p class="quiz-desc" style="font-size:12px; color: var(--texto2-color);">5 preguntas · Mínimo 50% para completar · <%= completado ? "✅ Ya dominas esta cocina — puedes repetir para perfeccionar" : "🥫 Disponible cuando domines las fuerzas dipolo-dipolo" %></p>
                </div>
    ...
    <script>
        ...
        function moverGotas() {
            for (let i = 0; i < gotas.length; i++) {
                ...
                g.x += g.vx;
                g.y += g.vy;
                if (g.x < 15) { g.x = 15; g.vx *= -0.9; }
                if (g.x > anchoGotas - 15) { g.x = anchoGotas - 15; g.vx *= -0.9; }
                if (g.y < 15) { g.y = 15; g.vy *= -0.9; }
                if (g.y > altoGotas - 15) { g.y = altoGotas - 15; g.vy *= -0.9; }
            }
        }
        ...
        function pausarGotas() { runGotas = false; cancelAnimationFrame(animGotasId); }
        function reanudarGotas() { if (!runGotas) { runGotas = true; loopGotas(); } }
        function reiniciarGotas() { pausarGotas(); initGotas(); reanudarGotas(); }
        loopGotas();
        ...
        function drop(e, slotId) {
            ...
            if (colocados[slotId]) {
                ...
                if (prev) document.getElementById('pool').appendChild(prev);
            }
