...
<script>
    ...
    let particulas = [];
    for (let i=0;i<20;i++) particulas.push({x:Math.random()*canvasEstados.width,y:Math.random()*canvasEstados.height,vx:(Math.random()-0.5)*2,vy:(Math.random()-0.5)*2,radio:4});

    function actualizarTemperatura(valor) {
        temperatura=parseInt(valor); document.getElementById('tempLabel').textContent=valor+' °C';
        let estado='Sólido';
        if (temperatura>0&&temperatura<100) estado='Líquido';
        else if (temperatura>=100) estado='Gas';
    ...
    function animar() {
        ...
        particulas.forEach(p => {
            ...
            if (vel>vMax){p.vx=(p.vx/vel)*vMax;p.vy=(p.vy/vel)*vMax;}
            if (temperatura<=0){let px=(p.x%100)+10,py=(p.y%100)+10;p.vx+=(px-p.x)*0.01;p.vy+=(py-p.y)*0.01;}
            p.x+=p.vx; p.y+=p.vy;
            if (p.x<0||p.x>canvasEstados.width) p.vx*=-1;
            if (p.y<0||p.y>canvasEstados.height) p.vy*=-1;
            ctxEstados.beginPath(); ctxEstados.arc(p.x,p.y,p.radio,0,Math.PI*2);
            ctxEstados.fillStyle='#4CAF50'; ctxEstados.fill();
        });
    ...
    function seleccionarOpcion(opcion, event) {
        opcionSeleccionada=opcion; document.getElementById('btnEnviarEstados').disabled=false;
