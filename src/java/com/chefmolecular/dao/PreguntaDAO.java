...
public class PreguntaDAO {
    ...
    public List<Pregunta> listarPorEscenario(int idEscenario) throws SQLException {
        ...
        System.out.println("    [BD] listarPorEscenario (preguntas): " + (System.currentTimeMillis() - inicio) + " ms");
        return lista;
    }
    ...
    public Pregunta buscarPorId(int idPregunta) throws SQLException {
        ...
        try (Connection cn = ConexionDB.obtener(); PreparedStatement ps = cn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    Pregunta p = mapear(rs);
                    System.out.println("    [BD] buscarPreguntaPorId: " + (System.currentTimeMillis() - inicio) + " ms");
                    return p;
                }
            }
        }
        ...
        try (Connection cn = ConexionDB.obtener(); PreparedStatement ps = cn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("    [BD] contarPorEscenario (preguntas): " + (System.currentTimeMillis() - inicio) + " ms");
                    return resultado;
                }
            }
        }
